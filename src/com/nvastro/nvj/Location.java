  ...
  static {
    ...
    for ( int i = 0; i < tzstrings.length; i++ ) {
      if ( tzstrings[i].indexOf('/') > 0 ) // If string/string
        // Associate string with number of minutes from GMT
        hash.put(tzstrings[i], Integer.valueOf(
            TimeZone.getTimeZone(tzstrings[i]).getRawOffset() / 60000));
    }
...
.  *
   ...
   *
   * @return Untrimmed city String, or "" if not set
   */
   ...
   *
   * @return Original untrimmed city String, or "" if not set
   */
   ...
   *
   * @return City String (or latitude and longitude if city not set)
   */
   ...
   *
   * @return Time zone offset in minutes (ignores DST)
   */
   ...
   * It is negative for west of Greenwich, and ignores DST.
   *
   * @return Time zone offset String (ignores DST)
   */
