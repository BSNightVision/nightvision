...
//static final private int YR0 = 1000;
  // Let 1st selectable year be 10 BC which would be -9
  // since there is no year 0
  static final private int YR0 = -9; // 10 BC
  ...
  private DateTimeDlg(Frame owner, Preferences prefer) {
    ...
    year  = new EComboBox();
      for ( i = YR0; i <= YRL; i++ ) {
    	if ( i < 1 ) year.addItem(Integer.toString(1 - i) + " BC");
    	else year.addItem(Integer.toString(i));
      }
  ...
  private int daysInMonth(int month, int year) { // month is 0 based
    ...
    if ( (month % 2) == 0 ) return 31;
      ...
      // Note: gc.isLeapYear(year) requires year <= 0 for BC
  ...
  private void loadPrefer() {
    ...
    if ( gc.get(Calendar.ERA) == GregorianCalendar.BC ) {
      yr = 1 - yr; // 1 BC -> 0, 2 BC -> -1, 3 BC -> -2, ...
    }
    int boundedyear = Math.max(YR0, Math.min(yr, YRL));
