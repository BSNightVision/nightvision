...
public class LST {
   ...
   *
   * @param btn JToggleButton to update the state
   */
   ...
   *
   * @return The Julian Ephemeris Day
   */
  ...
  public String tellLocDateTime(boolean ampm) {
    ...
    String era = "";
    if ( gc.get(Calendar.ERA) == GregorianCalendar.BC ) era = " BC";
    ...
    return String.format("%04d%s%02d%s%02d %02d%s%02d%s%s",
        year, DTSEP, mon, DTSEP, day, hr, TMSEP, mn, ampms, era);
  }
  ...
  public void setToCompDateTime() {
    ...
    gc.set(Calendar.ERA,         now.get(Calendar.ERA));
   ...
   *
   * @return Time zone offset relative to Greenwich (includes DST)
   */
   ...
   *
   * @return Name of city, or latitude and longitude if city not set
   */
   ...
   *
   * @return Name of city (or "" if not set)
   */
   ...
   *
   * @return Time zone offset in minutes relative to Greenwich (ignores DST)
   */
   ...
   * It is negative for west of Greenwich, and ignores DST.
   *
   * @return Time zone offset as String relative to Greenwich (ignores DST)
   */
