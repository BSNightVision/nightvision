...
public class SSWin extends JFrame implements ActionListener,
  ...
  private SSWin(JComponent starwin, Preferences prefer) {
    ...
    JMenuItem save_pref = new JMenuItem(TextBndl.getString("M.File.Save_pref"));
    save_pref.setActionCommand("save_pref");
    save_pref.addActionListener(this);
    file.add(save_pref);
    JMenuItem save = new JMenuItem(TextBndl.getString("M.File.Save_win"));
    save.setActionCommand("save_win");
    ...
    showscroll = prefer.getSSShowScrl();  // false means scrollbars
                 // aren't showing and menu item says "Show scrollbars"
    scroll = new JMenuItem(TextBndl.getString(showscroll ?
                           "M.Opt.ScrollOff" : "M.Opt.ScrollOn"));
    scroll.setActionCommand("scroll");
    scroll.addActionListener(this);
    showtime = prefer.getSSShowTime(); // true means time controls
               // are showing and menu item says "Hide time controls"
    time = new JMenuItem(TextBndl.getString(showtime ?
                         "M.Opt.TimeCtrlOff" : "M.Opt.TimeCtrlOn"));
    ...
    tilt = TSCRLMAX / 6; // 60 degrees from perpendicular, 30 from plane
  ...
  private void addKeyBindings() {
    ...
    imap.put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.SHIFT_DOWN_MASK),
    		 "lineup");
    ...
    imap.put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.CTRL_DOWN_MASK),
    		 "pageup");
    ...
    imap.put(KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.SHIFT_DOWN_MASK),
    		 "zoomout");
  }
  ...
  private void paintSS(Graphics2D g) {
    ...
    // Include Pluto?
    int numPlanets = NumPlanets - (prefer.drawPluto() ? 0 : 1);
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if      ( cmd.equals("inner") ) {
      ...
      if ( prefer.saveSSPrefer(showscroll, showtime) ) {
  ...
  /** <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   * Main starting point to show just the Solar System window.
   *
   * @param args User supplied command line argurments
   */
